  ...
  if (strlen($invoice) == 14) {
    ...
    $customer = $in->Tbl_Col_Id_LIMITE_0('customer','id',$payment->customerid);
     ...
     if ($_SERVER['REQUEST_METHOD'] == 'POST' AND isset($_POST['deleteval'])) {
        $id = $_POST['incomeid'];
        $product_Del = $in->Delete('income',$id);
        if ($product_Del) {
          echo "<meta http-equiv='refresh' content='0'>";
        }
     }
     if ($_SERVER['REQUEST_METHOD'] == 'POST' AND isset($_POST['addproductbyedit'])) {
       $edit_new_product = $in->edit_new_product($_POST);
     }
     if ($_SERVER['REQUEST_METHOD'] == 'POST' AND isset($_POST['editvalue225'])) {
       $edit_old_invoice = $in->edit_old_invoice($_POST);
       if ($edit_old_invoice) {
         $edit_old = "<p class='alert alert-success'><strong>Success !</strong> This Invoice Successfully Updated.</p>";
  ...
  <div class="">
      ...
      <div class="title_right">
        <div class="col-md-5 col-sm-5 col-xs-12 form-group pull-right top_search">
          <div class="input-group">
            <a href="<?php echo $invoice_page;?>.php?invoice=<?php echo $invoice;?>" class="btn btn-info">Return Invoice</a>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="row">
      <div class="col-md-12 col-sm-12 col-xs-12">
        <div class="x_panel">
          <div class="x_title">
            <h2>Invoice No. <span style="color: skyblue;"><?php echo $invoice;?></span></h2>
              ...
              <table class="table table-bordered">
                ...
                ?>
                  <tr>
                    ...
                    <td>
                      <form action="" method="post">
                        <input type="text" name="rate502" value="<?php echo $product_s->rate;?>" class="text-center" style="width: 90px;border-radius: 30%;padding: 6px;" data-toggle="tooltip" title="<?php echo $farjan->amountsymbol;?>">
                        <input type="hidden" name="proid" value="<?php echo $product_s->id;?>">
                        <input type="submit" name="rateupdate" value="Up" class="btn btn-info btn-xs">
                      </form>
                    </td>
                    <td>
                      <form action="" method="post">
                        <input type="text" name="quanval" autocomplete="off" value="<?php echo $product_s->quantity;?>" class="text-center" style="width: 90px;border-radius: 30%;padding: 6px;" data-toggle="tooltip" title="<?php echo $product_s->unit;?>">
                ...
                <form action="" method="post">
                    <tr>
                      ...
                      <td>
                        ...
                        <input type="hidden" name="productidnewadd" id="productidnewadd">
                      ...
                      <td>
                        <input type="text" name="rateadd" id="rateadd" class="form-control" data-toggle="tooltip" title="<?php echo $farjan->amountsymbol;?>">
                      </td>
                      <td>
                        <input type="text" name="qntadd" id="qntadd" class="form-control" data-toggle="tooltip">
                      </td>
                      <td>
...
<script type="text/javascript">
  $(function(){
    ...
    $("#productcode").blur(function(){
      ...
      $.ajax({
        url:"selectproductquantityedit.php",
        method:"POST",
        data:{ProductVal:productcode},
        success:function(data){
          var obj2 = $.parseJSON(data);
          $("#qntadd").val(obj2.total);
          $("#qntadd").attr("title",obj2.unit);
        }
      });
      rateqnt();
    });
