...
<?php
 ...
 if ($_SERVER['REQUEST_METHOD'] == "POST" AND isset($_POST['personcash'])) {
   $addcustomer = $cu->Person_Cash_In_List($_POST);
 }
      ...
      <div class="title_right">
        <div class="col-md-4 col-sm-5 col-xs-12 form-group pull-right top_search">
          <div class="input-group">
            <a href="bypersonlistin.php" class="btn btn-info">Loan Receive Statement</a>
          </div>
        </div>
      </div>
    </div>
...
<form class="form-horizontal form-label-left" action="" method="post" enctype="multipart/form-data">
  ...
  <div class="item form-group">
    ...
    <div class="col-md-6 col-sm-6 col-xs-12">
      ...
      <input type="hidden" id="diary" name="diary" class="form-control col-md-7 col-xs-12" >
    </div>
  </div>
...
$(document).on('click','#dataval ul li',function(){
  ...
  $.ajax({
        ...
        success: function(data){
          ...
          $("#amount").val(obj.payamount);
          $("#diary").val(obj.ByomjatrirDiary);
        }
      });
    $("#name").val(ulval);
    $("#showdata").slideUp(1000);
