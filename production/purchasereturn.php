...
$invoice = "";
 if ($_SERVER['REQUEST_METHOD'] == 'POST' AND isset($_POST['serachinvoice'])) {
   ...
   echo "<script>window.location = 'purchasereturn.php?invoice=".$invoice."';</script>";
 }
 if ($_SERVER['REQUEST_METHOD'] == 'POST' AND isset($_POST['upquant'])) {
  $upquant = $po->Update_Purchase_Product($_POST);
 }
 if ($_SERVER['REQUEST_METHOD'] == 'POST' AND isset($_POST['returnamval'])) {
  $Success = $po->Update_Payment_Return($_POST,'2');
 }
  ...
  <div class="">
    <div class="page-title">
      <div class="title_left">
        <h3>Purchase Return Page</h3>
      </div>

      <div class="title_right">
        <div class="col-md-5 col-sm-5 col-xs-12 form-group pull-right top_search">
          <div class="input-group">
            <a href="purchasereturn.php"><button class="btn btn-info">Refresh </button></a>
            <a href="purchasereturnlist.php"><button class="btn btn-primary">View All Return </button></a>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="row">
      <div class="col-md-12 col-sm-12 col-xs-12" <?php if (isset($payment)) {echo "hidden='hidden'";}?>>
        <div class="x_panel">
          <div class="x_title">
            <h2>Purchase Return <small></small></h2>
          ...
          <span id="message123"><?php if(isset($addproduct)){echo $addproduct;}elseif(isset($Up_product)){echo $Up_product;}elseif(isset($Success)){echo $Success;}?></span>
            ...
            <form id="demo-form2" data-parsley-validate class="form-horizontal form-label-left" action="" method="post">
              <div class="form-group">
                <label class="control-label col-md-3 col-sm-3 col-xs-12">Enter Purchase No <span class="required">*</span>
                </label>
        ...
        <div class="x_panel">
          ...
          <div class="x_content">
            ...
            <div class="maindiv">
              ...
              <?php
                $productall = $po->Tbl_Col_Id('purchase','purchaseno',$payment->invoice);
                $i = 0;
                $subtotal = 0;
                if ($productall) {
                  while ($data = $productall->fetch(PDO::FETCH_OBJ)) { $i++;
                    $prod = $po->Tbl_Col_Id_LIMITE_0('product','id',$data->productid);
                    $storeid = $po->Tbl_Col_Id_LIMITE_0('store','purchaseid',$data->id);
              ?>
                <tr>
                  <td><?php echo $i;?></td>
                  ...
                  <td>
                    <form action="" method="post">
                      <input type="text" name="quantity" value="<?php echo $total = $data->quantity;?>"  id="quantity">
                      <input type="hidden" name="incomeid" value="<?php echo $data->id;?>">
                      <input type="hidden" name="quantityold" value="<?php echo $data->quantity;?>">
                      <input type="hidden" name="returnold" value="<?php echo $data->returninfo;?>">
                      <input type="hidden" name="storeid" value="<?php echo $storeid->id;?>">
                      <input type="hidden" name="storereturnval" value="<?php echo $storeid->returninfo;?>">
                      <input type="submit" name="upquant" value="up" class="btn btn-primary btn-xs">
                    </form>
                  </td>
                  <td><?php if($data->unit == '0'){echo "sq/ft";}else{echo $data->unit;}?></td>
              ...
              <table class="result">
                ...
                <tr>
                  <td></td>
                  <td>
                    <input type="submit" name="returnamval" value="Return" class="btn btn-primary">
